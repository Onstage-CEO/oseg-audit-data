# Browser-capture task — The Phoenician Tavern

> **Generated handoff package — do not edit by hand.** Regenerate with
> `oseg-audit browser-capture-pack the-phoenician-tavern`. This is the bridge that
...
You are capturing live guest-review evidence for **The Phoenician Tavern** (Scottsdale, AZ, Luxury Collection / Marriott) using Joe's logged-in browser. Work the **required**
...
| Field | Value |
|---|---|
| Slug | `the-phoenician-tavern` |
| Property | The Phoenician Tavern |
| Location | Scottsdale, AZ |
| Brand | Luxury Collection / Marriott |
...
- **Run these searches:**
  - `The Phoenician Tavern Scottsdale, AZ reviews`
  - `The Phoenician Tavern entertainment`
  - `The Phoenician Tavern nightlife`
  - `The Phoenician Tavern live music`
  - `The Phoenician Tavern after dinner`
  - `The Phoenician Tavern evening`
  - `The Phoenician Tavern family`
  - `The Phoenician Tavern atmosphere`
...
- **Run these searches:**
  - `site:reddit.com The Phoenician Tavern review`
  - `site:reddit.com The Phoenician Tavern nightlife`
  - `site:reddit.com The Phoenician Tavern entertainment`
  - `site:reddit.com The Phoenician Tavern party`
  - `site:reddit.com The Phoenician Tavern Scottsdale, AZ`
...
- **Run these searches:**
  - `The Phoenician Tavern walkthrough`
  - `The Phoenician Tavern resort tour`
  - `The Phoenician Tavern vlog`
  - `The Phoenician Tavern Scottsdale, AZ tour`
...
- **Run these searches:**
  - `"The Phoenician Tavern" review blog`
  - `"The Phoenician Tavern" noise`
  - `"The Phoenician Tavern" honest review`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/the-phoenician-tavern/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/the-phoenician-tavern/findings.json --id-prefix <slugPrefix> -o data/clients/the-phoenician-tavern/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/the-phoenician-tavern/audit.json data/clients/the-phoenician-tavern/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/the-phoenician-tavern/audit.json --plan data/clients/the-phoenician-tavern/research-plan.json --capture-archive data/clients/the-phoenician-tavern/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/the-phoenician-tavern/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor the-phoenician-tavern` for the single next action.
   ```bash
   oseg-audit verify data/clients/the-phoenician-tavern/audit.json --capture-archive data/clients/the-phoenician-tavern/captures.json --summary
   ```
