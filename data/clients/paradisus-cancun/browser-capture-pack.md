# Browser-capture task — Paradisus Cancún

> **Generated handoff package — do not edit by hand.** Regenerate with
> `oseg-audit browser-capture-pack paradisus-cancun`. This is the bridge that
...
You are capturing live guest-review evidence for **Paradisus Cancún** (Cancun, Mexico, Meliã Hotels International) using Joe's logged-in browser. Work the **required**
...
| Field | Value |
|---|---|
| Slug | `paradisus-cancun` |
| Property | Paradisus Cancún |
| Location | Cancun, Mexico |
| Brand | Meliã Hotels International |
...
- **Run these searches:**
  - `Paradisus Cancún Cancun, Mexico reviews`
  - `Paradisus Cancún entertainment`
  - `Paradisus Cancún nightlife`
  - `Paradisus Cancún live music`
  - `Paradisus Cancún after dinner`
  - `Paradisus Cancún evening`
  - `Paradisus Cancún family`
  - `Paradisus Cancún atmosphere`
...
- **Run these searches:**
  - `site:reddit.com Paradisus Cancún review`
  - `site:reddit.com Paradisus Cancún nightlife`
  - `site:reddit.com Paradisus Cancún entertainment`
  - `site:reddit.com Paradisus Cancún party`
  - `site:reddit.com Paradisus Cancún Cancun, Mexico`
...
- **Run these searches:**
  - `Paradisus Cancún walkthrough`
  - `Paradisus Cancún resort tour`
  - `Paradisus Cancún vlog`
  - `Paradisus Cancún Cancun, Mexico tour`
...
- **Run these searches:**
  - `"Paradisus Cancún" review blog`
  - `"Paradisus Cancún" noise`
  - `"Paradisus Cancún" honest review`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/paradisus-cancun/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/paradisus-cancun/findings.json --id-prefix <slugPrefix> -o data/clients/paradisus-cancun/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/paradisus-cancun/audit.json data/clients/paradisus-cancun/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/paradisus-cancun/audit.json --plan data/clients/paradisus-cancun/research-plan.json --capture-archive data/clients/paradisus-cancun/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/paradisus-cancun/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor paradisus-cancun` for the single next action.
   ```bash
   oseg-audit verify data/clients/paradisus-cancun/audit.json --capture-archive data/clients/paradisus-cancun/captures.json --summary
   ```
