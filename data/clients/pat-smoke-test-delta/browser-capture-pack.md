# Browser-capture task — PAT Smoke Test Delta

> **Generated handoff package — do not edit by hand.** Regenerate with
> `oseg-audit browser-capture-pack pat-smoke-test-delta`. This is the bridge that
...
You are capturing live guest-review evidence for **PAT Smoke Test Delta** (Test, AZ, Test) using Joe's logged-in browser. Work the **required**
...
| Field | Value |
|---|---|
| Slug | `pat-smoke-test-delta` |
| Property | PAT Smoke Test Delta |
| Location | Test, AZ |
| Brand | Test |
...
- **Run these searches:**
  - `PAT Smoke Test Delta Test, AZ reviews`
  - `PAT Smoke Test Delta entertainment`
  - `PAT Smoke Test Delta nightlife`
  - `PAT Smoke Test Delta live music`
  - `PAT Smoke Test Delta after dinner`
  - `PAT Smoke Test Delta evening`
  - `PAT Smoke Test Delta family`
  - `PAT Smoke Test Delta atmosphere`
...
- **Run these searches:**
  - `site:reddit.com PAT Smoke Test Delta review`
  - `site:reddit.com PAT Smoke Test Delta nightlife`
  - `site:reddit.com PAT Smoke Test Delta entertainment`
  - `site:reddit.com PAT Smoke Test Delta party`
  - `site:reddit.com PAT Smoke Test Delta Test, AZ`
...
- **Run these searches:**
  - `PAT Smoke Test Delta walkthrough`
  - `PAT Smoke Test Delta resort tour`
  - `PAT Smoke Test Delta vlog`
  - `PAT Smoke Test Delta Test, AZ tour`
...
- **Run these searches:**
  - `"PAT Smoke Test Delta" review blog`
  - `"PAT Smoke Test Delta" noise`
  - `"PAT Smoke Test Delta" honest review`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/pat-smoke-test-delta/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/pat-smoke-test-delta/findings.json --id-prefix <slugPrefix> -o data/clients/pat-smoke-test-delta/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/pat-smoke-test-delta/audit.json data/clients/pat-smoke-test-delta/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/pat-smoke-test-delta/audit.json --plan data/clients/pat-smoke-test-delta/research-plan.json --capture-archive data/clients/pat-smoke-test-delta/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/pat-smoke-test-delta/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor pat-smoke-test-delta` for the single next action.
   ```bash
   oseg-audit verify data/clients/pat-smoke-test-delta/audit.json --capture-archive data/clients/pat-smoke-test-delta/captures.json --summary
   ```
