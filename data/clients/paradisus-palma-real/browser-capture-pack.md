# Browser-capture task — Paradisus Palma Real

> **Generated handoff package — do not edit by hand.** Regenerate with
> `oseg-audit browser-capture-pack paradisus-palma-real`. This is the bridge that
...
You are capturing live guest-review evidence for **Paradisus Palma Real** (Punta Cana, Dominican Republic) using Joe's logged-in browser. Work the **required**
...
- **Run these searches:**
  - `Paradisus Palma Real Punta Cana, Dominican Republic reviews`
  - `Paradisus Palma Real entertainment`
  - `Paradisus Palma Real nightlife`
  - `Paradisus Palma Real live music`
  - `Paradisus Palma Real after dinner`
  - `Paradisus Palma Real evening`
  - `Paradisus Palma Real family`
  - `Paradisus Palma Real atmosphere`
...
- **Run these searches:**
  - `site:reddit.com Paradisus Palma Real review`
  - `site:reddit.com Paradisus Palma Real nightlife`
  - `site:reddit.com Paradisus Palma Real entertainment`
  - `site:reddit.com Paradisus Palma Real party`
  - `site:reddit.com Paradisus Palma Real Punta Cana, Dominican Republic`
...
- **Run these searches:**
  - `Paradisus Palma Real walkthrough`
  - `Paradisus Palma Real resort tour`
  - `Paradisus Palma Real vlog`
  - `Paradisus Palma Real Punta Cana, Dominican Republic tour`
...
- **Run these searches:**
  - `"Paradisus Palma Real" review blog`
  - `"Paradisus Palma Real" noise`
  - `"Paradisus Palma Real" honest review`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/paradisus-palma-real/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/paradisus-palma-real/findings.json --id-prefix <slugPrefix> -o data/clients/paradisus-palma-real/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/paradisus-palma-real/audit.json data/clients/paradisus-palma-real/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/paradisus-palma-real/audit.json --plan data/clients/paradisus-palma-real/research-plan.json --capture-archive data/clients/paradisus-palma-real/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/paradisus-palma-real/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor paradisus-palma-real` for the single next action.
   ```bash
   oseg-audit verify data/clients/paradisus-palma-real/audit.json --capture-archive data/clients/paradisus-palma-real/captures.json --summary
   ```
