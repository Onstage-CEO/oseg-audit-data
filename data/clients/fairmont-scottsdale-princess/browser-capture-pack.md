# Browser-capture task — Fairmont Scottsdale Princess

> **Generated handoff package — do not edit by hand.** Regenerate with
> `oseg-audit browser-capture-pack fairmont-scottsdale-princess`. This is the bridge that
...
You are capturing live guest-review evidence for **Fairmont Scottsdale Princess** (Scottsdale, Arizona) using Joe's logged-in browser. Work the **required**
...
| Property | Fairmont Scottsdale Princess |
...
- **Run these searches:**
  - `Fairmont Scottsdale Princess Scottsdale, Arizona reviews`
  - `Fairmont Scottsdale Princess entertainment`
  - `Fairmont Scottsdale Princess nightlife`
  - `Fairmont Scottsdale Princess live music`
  - `Fairmont Scottsdale Princess after dinner`
  - `Fairmont Scottsdale Princess evening`
  - `Fairmont Scottsdale Princess family`
  - `Fairmont Scottsdale Princess atmosphere`
...
- **Run these searches:**
  - `site:reddit.com Fairmont Scottsdale Princess review`
  - `site:reddit.com Fairmont Scottsdale Princess nightlife`
  - `site:reddit.com Fairmont Scottsdale Princess entertainment`
  - `site:reddit.com Fairmont Scottsdale Princess party`
  - `site:reddit.com Fairmont Scottsdale Princess Scottsdale, Arizona`
...
- **Run these searches:**
  - `Fairmont Scottsdale Princess walkthrough`
  - `Fairmont Scottsdale Princess resort tour`
  - `Fairmont Scottsdale Princess vlog`
  - `Fairmont Scottsdale Princess Scottsdale, Arizona tour`
...
- **Run these searches:**
  - `"Fairmont Scottsdale Princess" review blog`
  - `"Fairmont Scottsdale Princess" noise`
  - `"Fairmont Scottsdale Princess" honest review`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/fairmont-scottsdale-princess/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/fairmont-scottsdale-princess/findings.json --id-prefix <slugPrefix> -o data/clients/fairmont-scottsdale-princess/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/fairmont-scottsdale-princess/audit.json data/clients/fairmont-scottsdale-princess/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/fairmont-scottsdale-princess/audit.json --plan data/clients/fairmont-scottsdale-princess/research-plan.json --capture-archive data/clients/fairmont-scottsdale-princess/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/fairmont-scottsdale-princess/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor fairmont-scottsdale-princess` for the single next action.
   ```bash
   oseg-audit verify data/clients/fairmont-scottsdale-princess/audit.json --capture-archive data/clients/fairmont-scottsdale-princess/captures.json --summary
   ```
