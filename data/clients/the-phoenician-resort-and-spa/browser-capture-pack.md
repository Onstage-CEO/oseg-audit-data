...
> `oseg-audit browser-capture-pack the-phoenician-resort-and-spa`. This is the bridge that
...
You are capturing live guest-review evidence for **The Phoenician Resort and Spa** (Scottsdale, AZ, Marriott) using Joe's logged-in browser. Work the **required**
...
| Field | Value |
|---|---|
| Slug | `the-phoenician-resort-and-spa` |
...
- **Run these searches:**
  - `The Phoenician Resort and Spa Scottsdale, AZ reviews`
...
- **Run these searches:**
  ...
  - `site:reddit.com The Phoenician Resort and Spa Scottsdale, AZ`
...
- **Run these searches:**
  ...
  - `The Phoenician Resort and Spa Scottsdale, AZ tour`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/the-phoenician-resort-and-spa/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/the-phoenician-resort-and-spa/findings.json --id-prefix <slugPrefix> -o data/clients/the-phoenician-resort-and-spa/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/the-phoenician-resort-and-spa/audit.json data/clients/the-phoenician-resort-and-spa/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/the-phoenician-resort-and-spa/audit.json --plan data/clients/the-phoenician-resort-and-spa/research-plan.json --capture-archive data/clients/the-phoenician-resort-and-spa/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/the-phoenician-resort-and-spa/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor the-phoenician-resort-and-spa` for the single next action.
   ```bash
   oseg-audit verify data/clients/the-phoenician-resort-and-spa/audit.json --capture-archive data/clients/the-phoenician-resort-and-spa/captures.json --summary
   ```
