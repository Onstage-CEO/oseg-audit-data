# Browser-capture task — Test Resort Gamma

> **Generated handoff package — do not edit by hand.** Regenerate with
> `oseg-audit browser-capture-pack test-resort-gamma`. This is the bridge that
...
You are capturing live guest-review evidence for **Test Resort Gamma** (Sedona, Arizona) using Joe's logged-in browser. Work the **required**
...
| Field | Value |
|---|---|
| Slug | `test-resort-gamma` |
| Property | Test Resort Gamma |
...
- **Run these searches:**
  - `Test Resort Gamma Sedona, Arizona reviews`
  - `Test Resort Gamma entertainment`
  - `Test Resort Gamma nightlife`
  - `Test Resort Gamma live music`
  - `Test Resort Gamma after dinner`
  - `Test Resort Gamma evening`
  - `Test Resort Gamma family`
  - `Test Resort Gamma atmosphere`
...
- **Run these searches:**
  - `site:reddit.com Test Resort Gamma review`
  - `site:reddit.com Test Resort Gamma nightlife`
  - `site:reddit.com Test Resort Gamma entertainment`
  - `site:reddit.com Test Resort Gamma party`
  - `site:reddit.com Test Resort Gamma Sedona, Arizona`
...
- **Run these searches:**
  - `Test Resort Gamma walkthrough`
  - `Test Resort Gamma resort tour`
  - `Test Resort Gamma vlog`
  - `Test Resort Gamma Sedona, Arizona tour`
...
- **Run these searches:**
  - `"Test Resort Gamma" review blog`
  - `"Test Resort Gamma" noise`
  - `"Test Resort Gamma" honest review`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/test-resort-gamma/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/test-resort-gamma/findings.json --id-prefix <slugPrefix> -o data/clients/test-resort-gamma/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/test-resort-gamma/audit.json data/clients/test-resort-gamma/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/test-resort-gamma/audit.json --plan data/clients/test-resort-gamma/research-plan.json --capture-archive data/clients/test-resort-gamma/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/test-resort-gamma/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor test-resort-gamma` for the single next action.
   ```bash
   oseg-audit verify data/clients/test-resort-gamma/audit.json --capture-archive data/clients/test-resort-gamma/captures.json --summary
   ```
