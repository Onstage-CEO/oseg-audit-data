# Browser-capture task — Caesars Republic Scottsdale

> **Generated handoff package — do not edit by hand.** Regenerate with
> `oseg-audit browser-capture-pack caesars-republic-scottsdale`. This is the bridge that
...
You are capturing live guest-review evidence for **Caesars Republic Scottsdale** (Scottsdale, Arizona, Hilton) using Joe's logged-in browser. Work the **required**
...
| Field | Value |
|---|---|
| Slug | `caesars-republic-scottsdale` |
| Property | Caesars Republic Scottsdale |
...
- **Run these searches:**
  - `Caesars Republic Scottsdale Scottsdale, Arizona reviews`
  - `Caesars Republic Scottsdale entertainment`
  - `Caesars Republic Scottsdale nightlife`
  - `Caesars Republic Scottsdale live music`
  - `Caesars Republic Scottsdale after dinner`
  - `Caesars Republic Scottsdale evening`
  - `Caesars Republic Scottsdale family`
  - `Caesars Republic Scottsdale atmosphere`
...
- **Run these searches:**
  - `site:reddit.com Caesars Republic Scottsdale review`
  - `site:reddit.com Caesars Republic Scottsdale nightlife`
  - `site:reddit.com Caesars Republic Scottsdale entertainment`
  - `site:reddit.com Caesars Republic Scottsdale party`
  - `site:reddit.com Caesars Republic Scottsdale Scottsdale, Arizona`
...
- **Run these searches:**
  - `Caesars Republic Scottsdale walkthrough`
  - `Caesars Republic Scottsdale resort tour`
  - `Caesars Republic Scottsdale vlog`
  - `Caesars Republic Scottsdale Scottsdale, Arizona tour`
...
- **Run these searches:**
  - `"Caesars Republic Scottsdale" review blog`
  - `"Caesars Republic Scottsdale" noise`
  - `"Caesars Republic Scottsdale" honest review`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/caesars-republic-scottsdale/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/caesars-republic-scottsdale/findings.json --id-prefix <slugPrefix> -o data/clients/caesars-republic-scottsdale/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/caesars-republic-scottsdale/audit.json data/clients/caesars-republic-scottsdale/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/caesars-republic-scottsdale/audit.json --plan data/clients/caesars-republic-scottsdale/research-plan.json --capture-archive data/clients/caesars-republic-scottsdale/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/caesars-republic-scottsdale/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor caesars-republic-scottsdale` for the single next action.
   ```bash
   oseg-audit verify data/clients/caesars-republic-scottsdale/audit.json --capture-archive data/clients/caesars-republic-scottsdale/captures.json --summary
   ```
