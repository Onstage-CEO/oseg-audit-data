# Browser-capture task — The Scott Resort & Spa

> **Generated handoff package — do not edit by hand.** Regenerate with
> `oseg-audit browser-capture-pack the-scott-resort-spa`. This is the bridge that
...
You are capturing live guest-review evidence for **The Scott Resort & Spa** (Scottsdale, Arizona) using Joe's logged-in browser. Work the **required**
...
- **Run these searches:**
  - `The Scott Resort & Spa Scottsdale, Arizona reviews`
  - `The Scott Resort & Spa entertainment`
  - `The Scott Resort & Spa nightlife`
  - `The Scott Resort & Spa live music`
  - `The Scott Resort & Spa after dinner`
  - `The Scott Resort & Spa evening`
  - `The Scott Resort & Spa family`
  - `The Scott Resort & Spa atmosphere`
...
- **Run these searches:**
  - `site:reddit.com The Scott Resort & Spa review`
  - `site:reddit.com The Scott Resort & Spa nightlife`
  - `site:reddit.com The Scott Resort & Spa entertainment`
  - `site:reddit.com The Scott Resort & Spa party`
  - `site:reddit.com The Scott Resort & Spa Scottsdale, Arizona`
...
- **Run these searches:**
  - `The Scott Resort & Spa walkthrough`
  - `The Scott Resort & Spa resort tour`
  - `The Scott Resort & Spa vlog`
  - `The Scott Resort & Spa Scottsdale, Arizona tour`
...
- **Run these searches:**
  - `"The Scott Resort & Spa" review blog`
  - `"The Scott Resort & Spa" noise`
  - `"The Scott Resort & Spa" honest review`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/the-scott-resort-spa/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/the-scott-resort-spa/findings.json --id-prefix <slugPrefix> -o data/clients/the-scott-resort-spa/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/the-scott-resort-spa/audit.json data/clients/the-scott-resort-spa/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/the-scott-resort-spa/audit.json --plan data/clients/the-scott-resort-spa/research-plan.json --capture-archive data/clients/the-scott-resort-spa/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/the-scott-resort-spa/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor the-scott-resort-spa` for the single next action.
   ```bash
   oseg-audit verify data/clients/the-scott-resort-spa/audit.json --capture-archive data/clients/the-scott-resort-spa/captures.json --summary
   ```
