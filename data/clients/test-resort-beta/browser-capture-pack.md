# Browser-capture task — Test Resort Beta

> **Generated handoff package — do not edit by hand.** Regenerate with
> `oseg-audit browser-capture-pack test-resort-beta`. This is the bridge that
...
You are capturing live guest-review evidence for **Test Resort Beta** (Tucson, Arizona) using Joe's logged-in browser. Work the **required**
...
| Field | Value |
|---|---|
| Slug | `test-resort-beta` |
| Property | Test Resort Beta |
...
- **Run these searches:**
  - `Test Resort Beta Tucson, Arizona reviews`
  - `Test Resort Beta entertainment`
  - `Test Resort Beta nightlife`
  - `Test Resort Beta live music`
  - `Test Resort Beta after dinner`
  - `Test Resort Beta evening`
  - `Test Resort Beta family`
  - `Test Resort Beta atmosphere`
...
- **Run these searches:**
  - `site:reddit.com Test Resort Beta review`
  - `site:reddit.com Test Resort Beta nightlife`
  - `site:reddit.com Test Resort Beta entertainment`
  - `site:reddit.com Test Resort Beta party`
  - `site:reddit.com Test Resort Beta Tucson, Arizona`
...
- **Run these searches:**
  - `Test Resort Beta walkthrough`
  - `Test Resort Beta resort tour`
  - `Test Resort Beta vlog`
  - `Test Resort Beta Tucson, Arizona tour`
...
- **Run these searches:**
  - `"Test Resort Beta" review blog`
  - `"Test Resort Beta" noise`
  - `"Test Resort Beta" honest review`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/test-resort-beta/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/test-resort-beta/findings.json --id-prefix <slugPrefix> -o data/clients/test-resort-beta/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/test-resort-beta/audit.json data/clients/test-resort-beta/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/test-resort-beta/audit.json --plan data/clients/test-resort-beta/research-plan.json --capture-archive data/clients/test-resort-beta/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/test-resort-beta/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor test-resort-beta` for the single next action.
   ```bash
   oseg-audit verify data/clients/test-resort-beta/audit.json --capture-archive data/clients/test-resort-beta/captures.json --summary
   ```
