# Browser-capture task — UI Smoke Test Echo

> **Generated handoff package — do not edit by hand.** Regenerate with
> `oseg-audit browser-capture-pack ui-smoke-test-echo`. This is the bridge that
...
You are capturing live guest-review evidence for **UI Smoke Test Echo** (Test, AZ) using Joe's logged-in browser. Work the **required**
...
| Field | Value |
|---|---|
| Slug | `ui-smoke-test-echo` |
| Property | UI Smoke Test Echo |
| Location | Test, AZ |
...
- **Run these searches:**
  - `UI Smoke Test Echo Test, AZ reviews`
  - `UI Smoke Test Echo entertainment`
  - `UI Smoke Test Echo nightlife`
  - `UI Smoke Test Echo live music`
  - `UI Smoke Test Echo after dinner`
  - `UI Smoke Test Echo evening`
  - `UI Smoke Test Echo family`
  - `UI Smoke Test Echo atmosphere`
...
- **Run these searches:**
  - `site:reddit.com UI Smoke Test Echo review`
  - `site:reddit.com UI Smoke Test Echo nightlife`
  - `site:reddit.com UI Smoke Test Echo entertainment`
  - `site:reddit.com UI Smoke Test Echo party`
  - `site:reddit.com UI Smoke Test Echo Test, AZ`
...
- **Run these searches:**
  - `UI Smoke Test Echo walkthrough`
  - `UI Smoke Test Echo resort tour`
  - `UI Smoke Test Echo vlog`
  - `UI Smoke Test Echo Test, AZ tour`
...
- **Run these searches:**
  - `"UI Smoke Test Echo" review blog`
  - `"UI Smoke Test Echo" noise`
  - `"UI Smoke Test Echo" honest review`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/ui-smoke-test-echo/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/ui-smoke-test-echo/findings.json --id-prefix <slugPrefix> -o data/clients/ui-smoke-test-echo/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/ui-smoke-test-echo/audit.json data/clients/ui-smoke-test-echo/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/ui-smoke-test-echo/audit.json --plan data/clients/ui-smoke-test-echo/research-plan.json --capture-archive data/clients/ui-smoke-test-echo/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/ui-smoke-test-echo/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor ui-smoke-test-echo` for the single next action.
   ```bash
   oseg-audit verify data/clients/ui-smoke-test-echo/audit.json --capture-archive data/clients/ui-smoke-test-echo/captures.json --summary
   ```
