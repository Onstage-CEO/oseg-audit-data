# Browser-capture task — Caesar's Republic

> **Generated handoff package — do not edit by hand.** Regenerate with
> `oseg-audit browser-capture-pack caesar-s-republic`. This is the bridge that
...
You are capturing live guest-review evidence for **Caesar's Republic** (Scottsdale, AZ, Caesar's) using Joe's logged-in browser. Work the **required**
...
| Field | Value |
|---|---|
| Slug | `caesar-s-republic` |
| Property | Caesar's Republic |
...
- **Run these searches:**
  - `Caesar's Republic Scottsdale, AZ reviews`
  - `Caesar's Republic entertainment`
  - `Caesar's Republic nightlife`
  - `Caesar's Republic live music`
  - `Caesar's Republic after dinner`
  - `Caesar's Republic evening`
  - `Caesar's Republic family`
  - `Caesar's Republic atmosphere`
...
- **Run these searches:**
  - `site:reddit.com Caesar's Republic review`
  - `site:reddit.com Caesar's Republic nightlife`
  - `site:reddit.com Caesar's Republic entertainment`
  - `site:reddit.com Caesar's Republic party`
  - `site:reddit.com Caesar's Republic Scottsdale, AZ`
...
- **Run these searches:**
  - `Caesar's Republic walkthrough`
  - `Caesar's Republic resort tour`
  - `Caesar's Republic vlog`
  - `Caesar's Republic Scottsdale, AZ tour`
...
- **Run these searches:**
  - `"Caesar's Republic" review blog`
  - `"Caesar's Republic" noise`
  - `"Caesar's Republic" honest review`
...
1. **capture-record** — Record provenance for each artifact you captured (one per source page). Offline — only hashes the local screenshot.
   ```bash
   oseg-audit capture-record --source-url "<live url>" --platform <platform> --property "<name>" --location "<location>" --local-path <screenshot> --reviewer "<you>" -o data/clients/caesar-s-republic/captures.json
   ```
2. **research-intake** — Turn your authored findings (verbatim quotes + themes, tied to the captured URLs) into an evidence patch.
   ```bash
   oseg-audit research-intake data/clients/caesar-s-republic/findings.json --id-prefix <slugPrefix> -o data/clients/caesar-s-republic/evidence-patch.json
   ```
3. **merge-evidence** — Merge the new evidence into the audit.
   ```bash
   oseg-audit merge-evidence data/clients/caesar-s-republic/audit.json data/clients/caesar-s-republic/evidence-patch.json
   ```
4. **capture-coverage** — See which planned sources / theme slots / keywords still have no evidence — loop back to step 1 for the gaps.
   ```bash
   oseg-audit capture-coverage data/clients/caesar-s-republic/audit.json --plan data/clients/caesar-s-republic/research-plan.json --capture-archive data/clients/caesar-s-republic/captures.json
   ```
5. **promote-live-verified** — For each record you personally confirmed on the live page, promote it to live_verified (records HOW it was confirmed).
   ```bash
   oseg-audit promote-live-verified data/clients/caesar-s-republic/audit.json --evidence-id <id>
   ```
6. **verify** — The gate. Exit 0 = READY (teaser/deck unlock); nonzero = NOT READY — run `oseg-audit doctor caesar-s-republic` for the single next action.
   ```bash
   oseg-audit verify data/clients/caesar-s-republic/audit.json --capture-archive data/clients/caesar-s-republic/captures.json --summary
   ```
